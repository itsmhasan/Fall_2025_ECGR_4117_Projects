...
output_shape = 4
drop_rte = 0.5
hidden_neurons = 128 #[40, 40, 40, 40,output_shape]
...
for epoch in range(epochs):
    ...
    # train one epoch and record training loss
    epoch_train_loss = train_model(epoch, data, label)
    train_loss_hist.append(epoch_train_loss)
    if epoch % 1 == 0 and epoch != 0:
        with torch.no_grad():
            current_loss = test_model(test_data, test_label)
        test_loss_hist.append(current_loss)

        if current_loss < best_loss:
            best_loss = current_loss
            save_model(model)
    else:
        # for epoch == 0, we haven't run test_model yet, but we want array lengths to match.
        # we can either append NaN or run test_model once here.
        with torch.no_grad():
            current_loss = test_model(test_data, test_label)
        test_loss_hist.append(current_loss)

with torch.no_grad():
    final_test_loss = test_model(test_data, test_label)
