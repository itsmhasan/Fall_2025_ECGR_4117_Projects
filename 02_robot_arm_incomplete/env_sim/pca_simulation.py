...
def calculate_pca(data, k=2):
    ...
    # Convert the input numpy array to a PyTorch tensor (float32)
    data = torch.from_numpy(data).float()

    # Take the mean of each column (joint angle 0 and joint angle 1)
    # keepdim=True keeps it as shape (1,2) instead of (2,)
    data_mean = torch.mean(data, dim=0, keepdim=True)

    # Subtract the mean from every row to "center" the data around zero.
    # This is required for PCA, otherwise the first component might just
    # point toward the mean instead of the true direction of variation.
    data = data - data_mean

    # Perform Singular Value Decomposition on the centered data matrix.
    # data = U * diag(S) * V^T
    # - Rows of U * S tell us how each sample projects into principal component space
    # - Columns of V tell us the principal directions in the original variable space
    U, S, V = torch.svd(data)

    # ---- ADDED DEBUG INFO ----
    # principal axis in joint space (θ0, θ1) for PC #1:
    first_pc_dir = V[:, 0]  # shape [2], this is what you turn in
    print("===== PCA DEBUG =====")
    print("Mean angles: ", data_mean[0][0].item(), data_mean[0][1].item())
    print("First principal component direction [θ0, θ1]: ", first_pc_dir.tolist())
    print("======================")
...
#     data_mean = torch.mean(data, dim=0, keepdim=True)
#     data = data - data_mean
#     U, S, V = torch.svd(data) ###### Use PyTorch SVD To Extract U,S,V ######
#
#     return U[:, :k], data_mean[0][0], data_mean[0][1]
