...
output_shape = 4
drop_rte = 0.1
hidden_neurons = 40 #[40, 40, 40, 40,output_shape]
...
while True:
    ...
    if len(sprites) > 0 and num_steps_0 == 0 and num_steps_1 == 0 and mouse_state_bool:
        ...
        # === BEGIN: Added for Section 9 reporting ===
        # Compute standard deviations across the dropout samples
        theta0_std = np.std(lst_theta0)
        theta1_std = np.std(lst_theta1)

        # define the "final" theta_0, theta_1 estimate as the mean of the samples
        theta0_mean = np.mean(lst_theta0)
        theta1_mean = np.mean(lst_theta1)

        print("============================================")
        print("Dropout-based IK estimate at this target:")
        print("theta_0 (mean, in [0,2pi)) =", theta0_mean)
        print("theta_1 (mean, in [0,2pi)) =", theta1_mean)
        print("Std Dev theta_0 =", theta0_std)
        print("Std Dev theta_1 =", theta1_std)
        print("============================================")
